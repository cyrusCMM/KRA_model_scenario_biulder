...
def build_macro_driver_table(
    ...
    for y in years:
        ...
        # DERIVED IDENTITIES
        # IMPORTANT:
        # - nominal_gdp_growth is ALWAYS derived
        # - do NOT read workbook 'Nominal GDP growth' as the driver
        # ----------------------------------------------------
...
if __name__ == "__main__":
    print("=" * 80)
    print("MACRO IDENTITIES TEST")
    print("=" * 80)

    macro_input = pd.DataFrame({
        "Variable": [
            "Real GDP growth",
            "GDP deflator",
            "Nominal GDP growth",   # ignored in computation
            "CPI",
            "Wage rate (% change)",
            "Import Value Growth",
            "Export Value Growth",
        ...
        "2025/26": [0.05, 0.05, 0.10, 0.07, 0.06, 0.06, 0.05, 0.06, 0.08, 0.01, 0.04, 0.00],
        "2026/27": [0.05, 0.05, 0.10, 0.06, 0.06, 0.07, 0.05, 0.07, 0.05, 0.01, 0.05, 0.01],
        "2027/28": [0.05, 0.05, 0.10, 0.06, 0.06, 0.07, 0.05, 0.07, 0.04, 0.01, 0.05, 0.01],
    })

    df = build_macro_driver_table(macro_input)
    print(df)