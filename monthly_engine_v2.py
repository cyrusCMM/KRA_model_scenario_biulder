...
============================================================
MONTHLY ENGINE V3
Full-year proportional monthly disaggregation engine

Design philosophy
-----------------
- Build monthly shares from a reference fiscal year (typically 2024/25)
- Disaggregate full-year annual baseline forecasts across all 12 months
- Disaggregate annual scenario deltas across eligible months using the same shares
- Do NOT blend current-year actuals into the monthly baseline path
- Allow scenario impact to start from a chosen fiscal month (e.g. April = 10)

This version is designed for scenario simulation and management reporting,
not rolling actual-plus-remaining forecasting.
============================================================
"""
...
def _require_columns(df: pd.DataFrame, required: List[str], label: str) -> None:
    if df is None or not isinstance(df, pd.DataFrame):
        raise MonthlyEngineError(f"{label} is missing or not a DataFrame.")
...
def build_monthly_taxhead_actuals(
    ...
    """
    Retained mainly for diagnostics / compatibility.
    It does NOT drive the baseline monthly forecast in this full-year version.
    """
    ...
    if out.empty and all_heads is not None and fiscal_year is not None:
...
def append_aggregate_monthly_path(monthly_path_df: pd.DataFrame, value_col: str) -> pd.DataFrame:
    ...
    if "Source" in out.columns:
        out["Source"] = out["Source"].fillna("forecast")
...
def build_baseline_monthly_path(
    ...
    monthly_taxhead_actuals_df: Optional[pd.DataFrame],
    fiscal_year: str,
    months_loaded: int,
    ...
    """
    Full-year proportional baseline path.

    months_loaded and use_actuals are kept in the signature only
    for compatibility with upstream callers.
    """
    ...
    path["Actual YTD"] = 0.0
    path["Remaining Forecast Total"] = path["Annual Final Forecast"]
    path["Loaded Share"] = 0.0
    path["Remaining Share"] = 1.0
    path["Remaining Month Normalized Share"] = path["Monthly Share"]
    path["Actual Monthly"] = 0.0

    path["Baseline Monthly Forecast"] = (
        pd.to_numeric(path["Annual Final Forecast"], errors="coerce").fillna(0.0)
        * pd.to_numeric(path["Monthly Share"], errors="coerce").fillna(0.0)
    )
...
def allocate_annual_delta_to_months(
    ...
    scenario_start_month: Optional[int] = None,
) -> pd.DataFrame:
    """
    Allocate annual scenario delta across eligible months.

    scenario_start_month overrides allocate_remaining_only/months_loaded
    when supplied. Example: April = 10.
    """
    ...
    if scenario_start_month is not None:
        path["Eligible"] = path["Month Index"] >= int(scenario_start_month)
    elif allocate_remaining_only:
...
def run_monthly_baseline_pipeline(
    ...
    """
    Full-year baseline disaggregation pipeline.
    months_loaded and use_actuals are retained only for interface compatibility.
    """
    ...
    actual_ytd = pd.DataFrame({
        "Internal Tax Head": annual_detail_df["Internal Tax Head"].astype(str).str.strip().unique(),
        "Actual YTD": 0.0,
    })
...
if __name__ == "__main__":
    print("=" * 90)
    print("MONTHLY ENGINE V3 LOADED")
    print("=" * 90)